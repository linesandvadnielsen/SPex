...
#####LOOK AT GROUP 7 TATLIPO (NOT 0 IN C-region!!!!!!)#####
...
if option != "":
    ...
    try:
        ...
        #st.write(children_sorted[0])

        #placeholder_children = st.sidebar.empty()
        #counter_par += 1

        #for i in range(len(children_sorted)):
        #    button_child[i] = placeholder_children.checkbox("Go to parent group: " + children_sorted[i], key = counter_par)
        #    counter_par += 1
        #    if button_child:
        #        st.write(children_sorted[i])
        ...
        if tax_id in euk_tax_groups:
            ...
            if select_analyses == "Full signal peptide length distribution":
                ...
                st.write("**Set information content threshold**")
                max_bits,blank = st.columns(2)
                info_euk_n = math.log2(20)
                max_bits_plot = max_bits.number_input("Maximum information content shown on sequence logo", min_value = float(0), max_value = math.log2(20), value = info_euk_n, step = 0.2, key = 2)

                #Display logo
                MakeLogo(tax_id,"SP","n", lower_n, upper_n, scient_name, color_scheme, keep_zero, max_bits_plot)
                ...
                st.write("**Set information content threshold**")
                max_bits,blank = st.columns(2)
                info_euk_h = math.log2(20)
                max_bits_plot = max_bits.number_input("Maximum information content shown on sequence logo", min_value = float(0), max_value = math.log2(20), value = info_euk_h, step = 0.2, key = 1)

                MakeLogo(tax_id,"SP","h", lower_h, upper_h, scient_name, color_scheme, keep_zero, max_bits_plot)
                ...
                st.write("**Set information content threshold**")
                max_bits,blank = st.columns(2)
                max_bits_plot = max_bits.number_input("Maximum information content shown on sequence logo", min_value = float(0), max_value = math.log2(20), value = math.log2(20), step = 0.2, key = 3)

                MakeLogo(tax_id,"SP","c", lower_c, upper_c, scient_name, color_scheme, keep_zero, max_bits_plot)
                ...
                prepareDownloadSPs(group_number = str(tax_id), sp_type = "Sec SPI")
        ...
        #Prokaryotic groups
            ...
            if select_sp == "Sec SPI":
                ...
                if select_analyses == "Full signal peptide length distribution":
                    ...
                    st.write("**Set information content threshold**")
                    max_bits,blank = st.columns(2)
                    max_bits_plot = max_bits.number_input("Maximum information content shown on sequence logo", min_value = float(0), max_value = math.log2(20), value = math.log2(20), step = 0.2, key = 4)
                    ...
                    st.write("**Set information content threshold**")
                    max_bits,blank = st.columns(2)
                    max_bits_plot = max_bits.number_input("Maximum information content shown on sequence logo", min_value = float(0), max_value = math.log2(20), value = math.log2(20), step = 0.2, key = 5)
                    ...
                    st.write("**Set information content threshold**")
                    max_bits,blank = st.columns(2)
                    max_bits_plot = max_bits.number_input("Maximum information content shown on sequence logo", min_value = float(0), max_value = math.log2(20), value = math.log2(20), step = 0.2, key = 6)
                    ...
                    prepareDownloadSPs(group_number = str(tax_id), sp_type = select_sp)
            ...
            #Sec SPII analyses
                ...
                if select_analyses == "Full signal peptide length distribution":
                    ...
                    st.write("**Set information content threshold**")
                    max_bits,blank = st.columns(2)
                    max_bits_plot = max_bits.number_input("Maximum information content shown on sequence logo", min_value = float(0), max_value = math.log2(20), value = math.log2(20), step = 0.2, key = 7)

                    MakeLogo(tax_id,"LIPO","n", lower_n, upper_n, scient_name, color_scheme, keep_zero, max_bits_plot)
                    ...
                    st.write("**Set information content threshold**")
                    max_bits,blank = st.columns(2)
                    max_bits_plot = max_bits.number_input("Maximum information content shown on sequence logo", min_value = float(0), max_value = math.log2(20), value = math.log2(20), step = 0.2, key = 8)

                    MakeLogo(tax_id,"LIPO","h", lower_h, upper_h, scient_name, color_scheme, keep_zero, max_bits_plot)
            ...
            #Sec SPIII analyses
                ...
                if select_analyses == "Full signal peptide length distribution":
                    ...
                    st.write("**Set information content threshold**")
                    max_bits,blank = st.columns(2)
                    max_bits_plot = max_bits.number_input("Maximum information content shown on sequence logo", min_value = float(0), max_value = math.log2(20), value = math.log2(20), step = 0.2, key = 9)

                    MakeLogo(tax_id,"PILIN","n", lower_n, upper_n, scient_name, color_scheme, keep_zero, max_bits_plot)
            ...
            #Tat SPI analyses
                ...
                if select_analyses == "Full signal peptide length distribution":
                    ...
                    st.write("**Set information content threshold**")
                    max_bits,blank = st.columns(2)
                    max_bits_plot = max_bits.number_input("Maximum information content shown on sequence logo", min_value = float(0), max_value = math.log2(20), value = math.log2(20), step = 0.2, key = 10)

                    MakeLogo(tax_id,"TAT","n", lower_n, upper_n, scient_name, color_scheme, keep_zero, max_bits_plot)
                    ...
                    st.write("**Set information content threshold**")
                    max_bits,blank = st.columns(2)
                    max_bits_plot = max_bits.number_input("Maximum information content shown on sequence logo", min_value = float(0), max_value = math.log2(20), value = math.log2(20), step = 0.2, key = 11)

                    MakeLogo(tax_id,"TAT","h", lower_h, upper_h, scient_name, color_scheme, keep_zero, max_bits_plot)
                    ...
                    st.write("**Set information content threshold**")
                    max_bits,blank = st.columns(2)
                    max_bits_plot = max_bits.number_input("Maximum information content shown on sequence logo", min_value = float(0), max_value = math.log2(20), value = math.log2(20), step = 0.2, key = 12)

                    MakeLogo(tax_id,"TAT","c", lower_c, upper_c, scient_name, color_scheme, keep_zero, max_bits_plot)
            ...
            #Tat SPII analyses
                ...
                if select_analyses == "Full signal peptide length distribution":
                    ...
                    st.write("**Set information content threshold**")
                    max_bits,blank = st.columns(2)
                    max_bits_plot = max_bits.number_input("Maximum information content shown on sequence logo", min_value = float(0), max_value = math.log2(20), value = math.log2(20), step = 0.2, key = 13)

                    MakeLogo(tax_id,"TATLIPO","n", lower_n, upper_n, scient_name, color_scheme, keep_zero, max_bits_plot)
                    #except ValueError:
                    #    st.warning("Your selected value for lower part of the range is too small. Please try with a greater value.")
                    min_pos_h = ExtractAlignmentPosition(tax_id, "LIPO", "H")[0]
                    len_seq_h = ExtractAlignmentPosition(tax_id, "LIPO", "H")[1]
                    ...
                    st.write("**Set information content threshold**")
                    max_bits,blank = st.columns(2)
                    max_bits_plot = max_bits.number_input("Maximum information content shown on sequence logo", min_value = float(0), max_value = math.log2(20), value = math.log2(20), step = 0.2, key = 14)

                    MakeLogo(tax_id,"TATLIPO","h", lower_h, upper_h, scient_name, color_scheme, keep_zero, max_bits_plot)
